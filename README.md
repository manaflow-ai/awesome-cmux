...
> A curated, opinionated guide to the **[cmux](https://github.com/manaflow-ai/cmux)** ecosystem — the Ghostty-based macOS terminal built for AI coding agents. **190+ community projects** and growing. This list covers `manaflow-ai/cmux` only; for the Go connection multiplexer by soheilhy, see that repo directly.
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Launch Pi inside named cmux workspaces and expose slash commands for status pills, progress bars, logs, browser splits, and screen reads, combining workspace setup and live sidebar reporting in one small Pi package | TypeScript |
| [stegmannb/pi-agent-cmux](https://github.com/stegmannb/pi-agent-cmux) | Pi | Track Pi run summaries and push completion status into cmux notifications and sidebar pills, pairing automatic run-end reporting with passive skills that let Pi update status during builds, tests, and deploys | TypeScript |
...
| [Minoo7/cmux-hooks](https://github.com/Minoo7/cmux-hooks) | Multi | Fan out cmux-aware hooks to SSH hosts, Hermes, and omp/Pi, then relay agent activity back as local notifications and sidebar badges through a stdlib helper rather than per-agent hand-written scripts | Shell |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Give Pi a `/cmux progress` command and lifecycle-driven status updates inside cmux workspaces, so project presets, browser panes, and sidebar progress all share the same package configuration | TypeScript |
| **OpenCode** | | | |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Add `/cmux log` and lifecycle log writes to Pi sessions launched through cmux, making the same package handle project bootstrap, browser setup, sidebar status, and activity-feed breadcrumbs | TypeScript |
| **OpenCode** | | | |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Combine Pi workspace presets with notification settings, letting each project decide whether completions, browser previews, and pane layouts should be opened together rather than managed by separate helper scripts | TypeScript |
| [stegmannb/pi-agent-cmux](https://github.com/stegmannb/pi-agent-cmux) | Pi | Summarize every Pi run into a cmux notification, including changed files, searches, shell commands, duration, and error state, giving run-end alerts more context than plain completion hooks | TypeScript |
...
| [Minoo7/cmux-hooks](https://github.com/Minoo7/cmux-hooks) | Multi | Install cmux notification hooks across SSH hosts and local agent runtimes from one fanout script, then route Hermes and omp/Pi events through the same helper for consistent alert formatting | Shell |
| [Michael-Z-Freeman/antigravity-cmux-notify](https://github.com/Michael-Z-Freeman/antigravity-cmux-notify) | Antigravity | Wire Google Antigravity CLI hooks to cmux notifications through Antigravity's `hooks.json` contract, with stdout-safe JSON handling so the notification hook does not invalidate the agent runtime | Shell |
...
| [swayamg20/cmux-agent-orchestrator](https://github.com/swayamg20/cmux-agent-orchestrator) | Multi | Coordinate Claude Code and Codex work from Obsidian by linking live cmux surfaces to durable Markdown tasks and workflow states, providing a human-directed control plane that leaves terminal processes and provider sessions owned by cmux and the agents themselves | TypeScript |
...
| [dongsik93/crosstalk](https://github.com/dongsik93/crosstalk) | Multi | Fan one prompt into visible Claude, Codex, and Antigravity panes, collect deterministic agree/disagree markers, and support co-work assignments backed by temporary files instead of hidden broker state | Shell |
| [0xNekr/cmux-bus](https://github.com/0xNekr/cmux-bus) | Multi | Coordinate adjacent cmux panes through an append-only JSONL message bus with file ownership claims and escalation records, giving agents a lightweight audit trail without a daemon, MCP server, or database | Shell |
| **Claude Code** | | | |
...
| [albertlieyingadrian/cmux-multiplexer](https://github.com/albertlieyingadrian/cmux-multiplexer) | Claude Code | Teach one Claude Code session to spawn, brief, and monitor child workspaces over `cmux send` and `read-screen`, with each worker isolated in its own directory, git worktree, and MCP context | Python |
...
| [manaflow-ai/cmux-skills](https://github.com/manaflow-ai/cmux-skills) | Multi | Provide installable cmux skills for any Agent Skills-compatible runtime, covering CLI control, settings, browser automation, artifacts, workspace refs, and customization from one maintained public source | Python |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Open project-specific browser splits from Pi workspace presets and expose `/cmux browser` commands for URL, reload, and snapshot actions, keeping visual debugging in the same package as workspace launch | TypeScript |
| **OpenCode** | | | |
...
| [albertlieyingadrian/cmux-multiplexer](https://github.com/albertlieyingadrian/cmux-multiplexer) | Claude Code | Spawn child Claude Code workspaces from an orchestrator session, brief each one with a task, and isolate work in git worktrees so parallel research and implementation can proceed without focus stealing | Python |
| [chsm04/cmux-tower](https://github.com/chsm04/cmux-tower) | Claude Code | Define TOML workspace presets with role prompts, split layouts, and single/team/manual modes, then launch repeatable Claude Code workspaces from an interactive control tower instead of ad hoc shell history | Shell |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Open Pi sessions from project config, create browser and terminal panes, and expose status/progress commands from the same package, turning workspace setup into a reusable Pi install | TypeScript |
| **OpenCode** | | | |
...
| [dongsik93/crosstalk](https://github.com/dongsik93/crosstalk) | Multi | Run co-work assignments across Claude, Codex, and Antigravity panes, optionally pairing those panes with git worktrees so each agent can deliver a separate slice before verdict collection | Shell |
| [n-filatov/cmux-workspace](https://github.com/n-filatov/cmux-workspace) | Multi | Create cmux workspaces with fresh git worktrees and per-repo setup commands inferred into `.cmux-workspace.json`, replacing repeated branch/bootstrap steps with a committed project launcher | TypeScript |
...
| [swayamg20/cmux-agent-orchestrator](https://github.com/swayamg20/cmux-agent-orchestrator) | Multi | Monitor Claude Code and Codex sessions across cmux workspaces from Obsidian, combining bounded terminal previews, notification-backed attention signals, conservative provider detection, and exact surface focus without persisting transcripts or claiming lifecycle states that the available evidence cannot prove | TypeScript |
...
| [Lumiwealth/cmux-agent-recovery](https://github.com/Lumiwealth/cmux-agent-recovery) | Multi | Record Claude Code and Codex resume metadata on each turn, then recover one restored cmux workspace at a time after crashes or updates without bulk-starting stale sessions blindly | Python |
...
| [NewTurn2017/cmux-remote](https://github.com/NewTurn2017/cmux-remote) | — | Control cmux from an iPhone over Tailscale with a SwiftUI client and Swift relay daemon, keeping terminal mirroring, key input, and workspace operations inside the user's private tailnet | Swift |
| [pallidev/cmux-relay](https://github.com/pallidev/cmux-relay) | — | Stream cmux terminal sessions to any device with a TypeScript relay agent and optional ACP chat integration, emphasizing phone-friendly monitoring for Claude Code, Codex, and Gemini panes | TypeScript |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | Pi | Define Pi workspace presets with named browser panes, terminal panes, notification policy, and project-specific commands, giving repeated cmux layouts a committed JSON configuration | TypeScript |
...
| [albertlieyingadrian/cmux-multiplexer](https://github.com/albertlieyingadrian/cmux-multiplexer) | `orchestrate` `worktree` `skill` | Teach one Claude Code session to spawn and monitor child cmux workspaces with worktree isolation | Python |
...
| [dmallory42/pi-cmux](https://github.com/dmallory42/pi-cmux) | `layout` `sidebar` `progress` `browser` | Launch configured Pi workspaces with browser panes, terminal panes, and sidebar commands | TypeScript |
| [stegmannb/pi-agent-cmux](https://github.com/stegmannb/pi-agent-cmux) | `notify` `sidebar` `skill` | Summarize Pi runs through cmux notifications and passive status/progress skills | TypeScript |
...
| [manaflow-ai/cmux-skills](https://github.com/manaflow-ai/cmux-skills) | `skill` `browser` `layout` `config` | Provide maintained cmux skills for CLI control, browser automation, artifacts, workspace refs, and settings | Python |
| [dongsik93/crosstalk](https://github.com/dongsik93/crosstalk) | `orchestrate` `worktree` `notify` | Coordinate Claude, Codex, and Antigravity panes with deterministic verdicts and co-work mode | Shell |
| [0xNekr/cmux-bus](https://github.com/0xNekr/cmux-bus) | `orchestrate` `logs` | Coordinate adjacent agents through JSONL events, handoffs, file ownership claims, and escalation records | Shell |
| [Lumiwealth/cmux-agent-recovery](https://github.com/Lumiwealth/cmux-agent-recovery) | `monitor` | Recover Claude Code and Codex sessions one workspace at a time after cmux restarts | Python |
| [NewTurn2017/cmux-remote](https://github.com/NewTurn2017/cmux-remote) | `monitor` `remote` | Mirror and control cmux from an iPhone over Tailscale with a SwiftUI client and relay daemon | Swift |
| [pallidev/cmux-relay](https://github.com/pallidev/cmux-relay) | `monitor` `remote` | Stream cmux terminal sessions to phones and other devices with a TypeScript relay agent | TypeScript |
| [Minoo7/cmux-hooks](https://github.com/Minoo7/cmux-hooks) | `notify` `sidebar` `config` | Fan out cmux hooks across SSH hosts, Hermes, and omp/Pi with one helper path | Shell |
| [n-filatov/cmux-workspace](https://github.com/n-filatov/cmux-workspace) | `worktree` `layout` `config` | Spawn cmux workspaces from git worktrees and committed per-repo setup commands | TypeScript |
| [gomipapa/cmux-sidecar](https://github.com/gomipapa/cmux-sidecar) | `browser` `layout` | Open code-server as a sidecar pane beside Claude Code or Codex sessions | Shell |
| [Michael-Z-Freeman/antigravity-cmux-notify](https://github.com/Michael-Z-Freeman/antigravity-cmux-notify) | `notify` | Send cmux notifications from Google Antigravity CLI hooks with stdout-safe JSON handling | Shell |
...
| [swayamg20/cmux-agent-orchestrator](https://github.com/swayamg20/cmux-agent-orchestrator) | `orchestrate` `monitor` | Coordinate and monitor Claude Code and Codex sessions from an Obsidian task board that links cmux surfaces to durable Markdown work, distinguishes runtime evidence from workflow status, and returns users to exact terminals while cmux retains process ownership | TypeScript |
...
| [wshmkr/astral-terminal](https://github.com/wshmkr/astral-terminal) | Build a Windows + WSL terminal inspired by cmux, with split panes, workspaces, browser panel, Claude Code hook notifications, restart-safe scrollback, and planned CLI scripting | TypeScript · ★6 |
...
| [no1msd/seance](https://github.com/no1msd/seance) | Build a GTK4 Linux terminal multiplexer that auto-detects Claude Code, Codex, and Pi sessions, tracking status, permission waits, task completions, desktop notifications, and unread state without dotfile setup | Zig · ★48 |
...
| [Octane0411/open-vibe-island](https://github.com/Octane0411/open-vibe-island) | Monitor Claude Code, Codex, and OpenCode from a native macOS menu-bar companion that can watch terminal, Ghostty, cmux, Kakoune, and iTerm workflows without depending on cmux APIs | Swift · ★1149 |
| [vakovalskii/codbash](https://github.com/vakovalskii/codbash) | Provide a browser dashboard for searching, replaying, tagging, and resuming Claude Code, Codex, Pi, Cursor, OpenCode, Kiro, and Copilot sessions, with cmux as one launch target | JavaScript · ★212 |
| [sverrirsig/claude-control](https://github.com/sverrirsig/claude-control) | Build a macOS dashboard for discovering Claude Code sessions, showing git and PR status, approving prompts, and focusing terminal tabs across iTerm2, Terminal, kitty, WezTerm, cmux, and others | TypeScript · ★119 |
| [ClipboardHealth/groundcrew](https://github.com/ClipboardHealth/groundcrew) | Dispatch Linear tickets to AI coding agents in sandboxed git worktrees with per-ticket status, resume tracking, and optional cmux workspace mapping for visible local execution | TypeScript · ★32 |
| [julo15/seshctl](https://github.com/julo15/seshctl) | Track terminal-based coding sessions across Terminal.app, iTerm2, VS Code, Cursor, Warp, Ghostty, and cmux through a native menu-bar app and CLI | Swift · ★5 |
